...
class BusStopTest {
    ...
    fun busStopTest() {
        ...
        val busStopString =
            moshi.adapter(FeatureCollection::class.java).toJson(busStopTree.getAllCollection())
        ...
        val nearestBusStop = FeatureTree(fovBusStopFeatureCollection).getNearestFeature(
            userGeometry.location,
            userGeometry.ruler
        )
        val busStopLocation = nearestBusStop!!.geometry as Point
        val distanceToBusStop =
            userGeometry.ruler.distance(userGeometry.location, busStopLocation.coordinates)
