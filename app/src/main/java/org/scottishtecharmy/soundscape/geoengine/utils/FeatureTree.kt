...
class FeatureTree(featureCollection: FeatureCollection?) {
    ...
    private fun distanceToEntry(entry: Entry<Feature, Geometry?>,
                                from: LngLatAlt,
                                ruler: CheapRuler) : Double {
        when (val p = entry.geometry()) {
            is Point -> {
                val position = LngLatAlt(p.x(), p.y())
                return from.distance(position, ruler)
            }
     ...
     */
    fun getNearbyCollection(location: LngLatAlt, distance: Double, ruler: CheapRuler): FeatureCollection {
        val featureCollection = FeatureCollection()
        if(tree != null) {
    ...
    fun getNearestCollectionWithinTriangle(triangle: Triangle,
                                           maxCount: Int,
                                           ruler: CheapRuler): FeatureCollection {
     ...
     */
    fun getNearestFeatureWithinTriangle(triangle: Triangle, ruler: CheapRuler): Feature? {
