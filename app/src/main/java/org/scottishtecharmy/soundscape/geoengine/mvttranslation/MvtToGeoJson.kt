...
fun vectorTileToGeoJson(tileX: Int,
    ...
    // Add intersections
    wayGenerator.generateWays(tileData[TreeId.INTERSECTIONS.id], tileData[TreeId.ROADS_AND_PATHS.id], intersectionMap, tileX, tileY, tileZoom)
    transitGenerator.generateWays(collection, tileData[TreeId.TRANSIT.id], intersectionMap, tileX, tileY, tileZoom)

    tileData[TreeId.ROADS.id] = getRoadsFeatureCollectionFromTileFeatureCollection(tileData[TreeId.ROADS_AND_PATHS.id])
